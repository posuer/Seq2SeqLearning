...
class ARCIITensor(BasicModel):
    def __init__(self, config):
        super(ARCIITensor, self).__init__(config)
        self.__name = 'ARCIITensor'
        ...
        self.setup(config)
        if not self.check():
            raise TypeError('[ARCIITensor] parameter check wrong')
        print('[ARCIITensor] init done', end='\n')
    ...
    def build(self):
        def conv2d_work(input_dim):
            ...
            for i in range(self.config['num_conv2d_layers']):
                seq.add(Conv2D(filters=self.config['2d_kernel_counts'][i],
                               kernel_size=self.config['2d_kernel_sizes'][i],
                               padding='same', activation='relu'))
                seq.add(MaxPooling2D(pool_size=(
                    self.config['2d_mpool_sizes'][i][0],
        ...
        q_conv = conv2d_work(self.config['embed_size'])(q_embed)

        d_conv = conv2d_work(self.config['embed_size'])(d_embed)
        ...
        cross = MatchTensorScore(self.config['embed_size'], 5, 'relu',
                                 kernel_regularizer=regularizers.l2(1e-4)
                                 )([q_conv, d_conv])
        show_layer_info('Match-plus', cross)

        z = Reshape(
            (self.config['text1_maxlen'], self.config['text2_maxlen'], -1))(
            cross)
